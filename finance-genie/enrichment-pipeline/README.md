...
The notebooks under `workshop/` push Delta Lake tables into Neo4j and pull enriched graph features back, but they require a live notebook kernel and manual execution. The scripts in `enrichment-pipeline/` wrap that same logic as Databricks Python tasks that run unattended via the CLI.
...
Edit `finance-genie/.env` and fill in all placeholder values. `enrichment-pipeline/`
...
Generates the synthetic fraud dataset and writes six files to `enrichment-pipeline/data/`:
...
Use this when `enrichment-pipeline/data/` already contains the six expected files and
...
All pipeline jobs are submitted as Databricks Python tasks using the same pattern from inside `enrichment-pipeline/`:
...
### 8. Run GDS Pipeline (one-shot setup for the deployed app)

Runs the GDS algorithms against Neo4j Aura. Runs locally using the `graphdatascience` Python client; no Databricks cluster required.

```bash
uv run setup/run_gds.py            # idempotent: no-op if all properties present
uv run setup/run_gds.py --force    # recompute and overwrite
```

This is the canonical entry point. Internally it delegates to `validation/run_gds.py` (the single source of truth for algorithm parameters) and adds an idempotency check up front: if every `:Account` node already has `risk_score`, `community_id`, `betweenness_centrality`, and `similarity_score`, the script exits 0 with a no-op message.

Writes the following node properties on every `:Account`:
...
| `similarity_score` | max JACCARD over `:SIMILAR_TO` edges from NodeSimilarity |

Also creates `:SIMILAR_TO` relationships and the `account_community_id` / `account_risk_score` lookup indexes. Run once per Aura dataset after `02_neo4j_ingest.py`. **The deployed `graph-fraud-analyst` app requires these properties to be populated.** Re-running on a fresh ingest is safe; re-running without `--force` on an already-populated graph is a no-op.
...
**This step is optional for the deployed `graph-fraud-analyst` app.** The deployed app reads ring, risky-account, and central-account data live from Neo4j via Cypher, and materializes per-session subgraphs to Delta at Load time. The gold tables exist for two narrower use cases: (1) the Genie quickstart, so Genie has tables to query before any Load action runs; (2) the fallback path if Aura is unreachable. Skip this step unless you need either case.
...
Diagnostic scripts that validate each pipeline stage. Run directly (not via CLI) from inside `enrichment-pipeline/`:
